...
from transformers import BertweetTokenizer, BertModel, BertTokenizer
...
bert_tokenizer.encode(train_df["text"][9000])
...
class bertRegressor(nn.Module):
    ...
    def forward(self, ids, attention_mask):
        pooled_out = self.bert(ids, attention_mask)[1]
...
def evaluate_model(model, dataLoader, device):
    ...
    return pearson_corr, label_final
...
corr_df.to_csv("corr_multi.csv", index=False)
...
test_df["multi_pred"] = label_scaler.inverse_transform(pred.reshape(-1, 1))
...
train_df["multi_pred"] = label_scaler.inverse_transform(train_pred.reshape(-1, 1))
