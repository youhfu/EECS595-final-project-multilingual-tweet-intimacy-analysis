...
# %%
bert_model = "cardiffnlp/twitter-xlm-roberta-base"
bert_tokenizer = XLMRobertaTokenizer.from_pretrained(bert_model)
...
class bertRegressor(nn.Module):
    def __init__(self, bert_model, dropout=0.1):
        ...
        self.bert = XLMRobertaForSequenceClassification.from_pretrained(
...
corr_df.to_csv("corr_xlm_T.csv", index=False)
...
test_df["xlm_t_pred"] = label_scaler.inverse_transform(pred.reshape(-1, 1))
...
train_df["xlm_t_pred"] = label_scaler.inverse_transform(train_pred.reshape(-1, 1))
